            ...
            <div class="form-grid">
              ...
              <div class="input-box">
                ...
                <select v-model="form.direccionDepartamental" :class="{ 'empty-select': !form.direccionDepartamental }">
                  <option value="" disabled>Dirección Departamental de Educación</option>
              ...
              <div class="field-row">
                ...
                <div class="input-box w-50">
                  ...
                  <select v-model="form.municipioNina" :class="{ 'empty-select': !form.municipioNina }" :disabled="!form.departamentoNina">
                    <option value="" disabled>{{ form.departamentoNina ? 'Municipio de residencia' : 'Selecciona un departamento primero' }}</option>
                    <option v-for="m in municipiosNina" :key="m.id" :value="m.id">{{ m.nombre }}</option>
                  </select>
                  <v-icon size="16" class="select-chevron">mdi-chevron-down</v-icon>
                </div>
              </div>
              ...
              <div class="field-row">
                <div class="input-box" :class="form.institucion ? 'w-50' : ''">
                  ...
                  <select v-model="form.institucion" :class="{ 'empty-select': !form.institucion }">
                    <option value="" disabled>Institución que reporta</option>
              ...
              <div class="field-row">
                <div class="input-box w-50">
                  ...
                  <select v-model="form.puebloPertenencia" :class="{ 'empty-select': !form.puebloPertenencia }">
                    <option value="" disabled>Pueblo de pertenencia</option>
                ...
                <div class="input-box w-50">
                  ...
                  <select v-model="form.comunidadLinguistica" :class="{ 'empty-select': !form.comunidadLinguistica }">
                    <option value="" disabled>Comunidad Lingüística</option>
          ...
          <div v-if="step === 3">
            ...
            <div class="form-grid">
              ...
              <div class="field-row">
                ...
                <div class="input-box w-50">
                  ...
                  <select v-model="form.statusActual" :class="{ 'empty-select': !form.statusActual }">
                    <option value="" disabled>Status actual dentro del Sistema</option>
              ...
              <div class="field-row">
                ...
                <div class="input-box w-50">
                  <v-icon size="16" class="input-icon">mdi-text-box-check-outline</v-icon>
              ...
              <div class="field-row">
                ...
                <div class="input-box w-50">
                  ...
                  <select v-model="form.anoEducativo" :class="{ 'empty-select': !form.anoEducativo }">
                    <option value="" disabled>Año del Ciclo Escolar</option>
                    <option v-for="a in ANOS" :key="a" :value="a">{{ a }}</option>
                  </select>
                  <v-icon size="16" class="select-chevron">mdi-chevron-down</v-icon>
                </div>
              ...
              <div class="field-row">
                <div class="input-box w-50">
                  ...
                  <select v-model="form.municipioEscuela" :class="{ 'empty-select': !form.municipioEscuela }" :disabled="!form.departamentoEscuela">
                    <option value="" disabled>{{ form.departamentoEscuela ? 'Municipio' : 'Selecciona un departamento primero' }}</option>
                    <option v-for="m in municipiosEscuela" :key="m.id" :value="m.id">{{ m.nombre }}</option>
                  </select>
                  <v-icon size="16" class="select-chevron">mdi-chevron-down</v-icon>
                </div>
                <div class="input-box w-50">
              ...
              <div class="field-row">
                ...
                <div class="input-box" style="flex: 1;">
                  <select v-model="form.jornada" :class="{ 'empty-select': !form.jornada }">
                    <option value="" disabled>Jornada</option>
                    <option v-for="jor in JORNADAS" :key="jor" :value="jor">{{ jor }}</option>
                  </select>
                  <v-icon size="16" class="select-chevron">mdi-chevron-down</v-icon>
                </div>
                <div class="input-box" style="flex: 1;">
              ...
              <div class="estado-wrapper">
                ...
                <div v-if="estadoOriginal" class="estado-actual-banner">
                  ...
                  <span class="estado-chip" :class="`chip-${estadoOriginal}`">{{ estadoOriginal }}</span>
                ...
                >
                  <v-icon
                    size="16"
                    class="input-icon"
                    :color="form.estado ? '#ff9797' : submitIntent ? '#e53935' : '#b0b0b0'"
                  >mdi-flag</v-icon>
...
const INSTITUCIONES = ['Ministerio Publico', 'Procuraduria General de la Nación', 'Otra']
...
const form = reactive({
  ...
  // Step 3
  codigoPersonal: '', statusActual: '', grado: '', nivel: '',
  resultado: '', anoEducativo: '', codigoUdi: '',
  departamentoEscuela: '', municipioEscuela: '', nombreCentro: '',
  direccionCentro: '', sector: '', jornada: '', area: '',
...
const cargarCaso = async (id) => {
  try {
    ...
    form.statusActual         = historial.status_actual || ''
    form.grado                = historial.grado || ''
    form.nivel                = historial.nivel || ''
    form.resultado            = historial.resultado || ''
...
.step-circle.active { background-color: #ff9797; color: #ffffff; }
.step-label       { font-size: 0.875rem; color: #b0b0b0; font-weight: 400; }
.step-label.active { color: #ff9797; font-weight: 600; }
.step-label.done   { color: #ff9797; }
.step-line        { flex: 1; height: 1px; background-color: #e0e0e0; margin: 0 1rem; }
.step-line.active { background-color: #ff9797; }
.form-card { background-color: #ffffff; border-radius: 0.75rem; padding: 2rem; border: 1px solid #f0f0f0; box-shadow: 0 1px 2px 0 rgba(0,0,0,0.05); }
.form-header { margin-bottom: 1.25rem; }
.form-header h2 { color: #6d6d6d; font-size: 1.0625rem; font-weight: 600; margin-bottom: 0.125rem; }
.text-pink-alert { color: #ff9797; font-size: 0.75rem; margin-bottom: 0.25rem; }
...
.btn-next { background-color: #ff9797; color: white; border: none; padding: 0.5rem 1.5rem; border-radius: 0.25rem; font-size: 0.875rem; cursor: pointer; transition: opacity 0.2s; }
...
.chip-pendiente  { background: #ffd6d6; color: #c05050; }
.chip-faltante   { background: #fef7e0; color: #b06000; }
.chip-completado { background: #e6f4ea; color: #137333; }
.estado-requerido {
  display: flex; align-items: center; gap: 0.3rem;
  color: #e53935; font-size: 0.72rem; font-weight: 500;
}
.numero-auto { background-color: #f5f5f5; }
.numero-auto input { color: #ff9797; font-weight: 700; cursor: default; letter-spacing: 0.05em; }
.update-banner {
